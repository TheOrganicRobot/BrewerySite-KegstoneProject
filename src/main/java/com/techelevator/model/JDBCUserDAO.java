...
public class JDBCUserDAO implements UserDAO {
	...
	public Object getUserByUserName(String userName) {	
		String sqlSearchForUsername = "SELECT * FROM app_user " + 
				"LEFT JOIN brewery_user ON app_user.user_id = brewery_user.user_id " +  
				"WHERE UPPER(user_name) = ?";
		...
		if(user.next()) {
			thisUser = new User();
			thisUser.setUserId(user.getLong("user_id"));  //Added for Brewer/Brewery connection
			thisUser.setUserName(user.getString("user_name"));
			thisUser.setPassword(user.getString("password"));
			thisUser.setRole(user.getString("role"));
		}
		//-----ADDED TO ATTACH BREWERY ID TO BREWER-----//
		if(thisUser.getRole().equals("brewer")) {
			thisUser.setBrewery_id(user.getLong("brewery_id"));
		}
		return thisUser;
	}
